...
func SavePostToSQL(post *model.Post) error {
	...
	err := conn.QueryRow(context.Background(),
		...
		post.PostOwnerID).Scan(&exists)
	if err != nil {
		return err
	}
	if !exists {
		return fmt.Errorf("user with ID %d does not exist", post.PostOwnerID)
	}
...
func SearchPostByID(postID uuid.UUID) (model.Post, error) {
	// connect to db
	conn := connectDB()
	defer conn.Close(context.Background())
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return model.Post{}, fmt.Errorf("no post found with ID %s", postID)
		}
		return model.Post{}, err // Return other errors that might have occurred
	}
